...
class SlideCompositor:
    ...
    def composite(self, video_path: str, slide_events: List[Dict],
        ...
        enc_args = self.ffmpeg.get_video_encoder()
        ...
        if n_chunks == 1:
            cmd = [
                ...
                '-map', '[out]', '-map', '0:a?',
                *enc_args, '-c:a', 'copy', '-r', '25', '-shortest',
                output_path,
            ]
            ...
            for ci in range(n_chunks):
                ...
                cmd = [
                    'ffmpeg', '-y', '-v', 'warning',
                    '-ss', str(ss), '-t', str(self.CHUNK_SECS),
                    '-i', video_path,
                    '-ss', str(ss), '-t', str(self.CHUNK_SECS),
                    '-i', slide_track_path,
                    '-filter_complex', filter_graph,
                ...
                self.ffmpeg.run(cmd, description=f'composite chunk {ci+1}/{n_chunks}')
